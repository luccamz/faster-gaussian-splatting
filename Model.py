...
from Optim.adam_utils import (
    replace_param_group_data,
    prune_param_groups,
    extend_param_groups,
    sort_param_groups,
    reset_state,
...
class Gaussians(torch.nn.Module):
    ...
    def setup_3d_filter(
        self, filter_config: Framework.ConfigParameterList, dataset: "BaseDataset"
    ) -> None:
        ...
        if filter_config.ORIGINAL_FORMULATION:
            ...
            Logger.log_info(
                f"using mip-splatting 3d filter with variance {filter_config.FILTER_VARIANCE}"
            )
        else:
            self.use_optimized_3d_filter = True
            Logger.log_info(
                f"using optimized 3d filter with variance {filter_config.FILTER_VARIANCE}"
            )
        max_focal = 1e-12
        for view in dataset:
            if not isinstance(view.camera, PerspectiveCamera):
                raise Framework.ModelError(
                    "update3dfilter only supports perspective cameras"
                )
    ...
    def compute_3d_filter(
        self, dataset: "BaseDataset", clipping_tolerance: float = 0.15
    ) -> None:
        ...
        visibility_mask = torch.zeros(
            (positions.shape[0], 1), device=positions.device, dtype=torch.bool
        )
        for view in dataset:
            if not isinstance(view.camera, PerspectiveCamera):
                raise Framework.ModelError(
        ...
        filter_3d = torch.where(
            visibility_mask, filter_3d, filter_3d_max, out=filter_3d
        )
        if self.use_original_3d_filter:
            filter_3d = (
                filter_3d.square()
            )  # original implementation always needs this in squared form
        elif self.use_optimized_3d_filter:
            filter_3d = (
                filter_3d.log()
            )  # optimized implementation uses this to directly clamp scales in logspace
        self._filter_3d = filter_3d

    def initialize_from_point_cloud(
        self, point_cloud: BasicPointCloud, use_mcmc: bool
    ) -> None:
        ...
        Logger.log_info(
            f"number of Gaussians at initialization: {n_initial_gaussians:,}"
        )
        # initial sh coefficients
        rgbs = (
            torch.full_like(means, fill_value=0.5)
            if point_cloud.colors is None
            else point_cloud.colors.cuda()
        )
        ...
        rotations = torch.zeros(
            (n_initial_gaussians, 4), dtype=torch.float32, device="cuda"
        )
        rotations[:, 0] = 1.0
        # initial opacities
        ...
        opacities = torch.full(
            (n_initial_gaussians, 1),
            fill_value=initial_opacity_logit,
            dtype=torch.float32,
            device="cuda",
        )
        # setup parameters
        ...
        self._sh_coefficients_rest = torch.nn.Parameter(
    ...
    def training_setup(self, training_wrapper, training_cameras_extent: float) -> None:
        ...
        param_groups = [
            {
                "params": [self._means],
                "lr": training_wrapper.OPTIMIZER.LEARNING_RATE_MEANS_INIT
                * self.training_cameras_extent,
                "name": "means",
            },
            {
        ...
        self.lr_means_scheduler = LRDecayPolicy(
            lr_init=training_wrapper.OPTIMIZER.LEARNING_RATE_MEANS_INIT
            * self.training_cameras_extent,
            lr_final=training_wrapper.OPTIMIZER.LEARNING_RATE_MEANS_FINAL
            * self.training_cameras_extent,
    ...
    def reset_opacities(self) -> None:
        ...
        opacities_new = self._opacities.clamp_max(
            -4.595119953155518
        )  # sigmoid(-4.595119953155518) = 0.01
        if self.use_original_3d_filter:
    ...
    def reset_densification_info(self, track_abs_grad: bool = False, track_pixel_counts: bool = False):
        # rows = [visibility count, gradient, abs-gradient (AbsGS split channel), pixel-coverage Sigma-p
        # (Pixel-GS clone denominator)]; the last two rows are present only when their feature is active.
        n_rows = 2 + int(track_abs_grad) + int(track_pixel_counts)
        self._densification_info = torch.zeros(
            (n_rows, self._means.shape[0]), dtype=torch.float32, device="cuda"
        )
        self.pixel_weighted = track_pixel_counts
    ...
    def adaptive_density_control(
        ...
        clone_denom = self.densification_info[2 + int(abs_grad_threshold is not None)].clamp_min(1.0) if self.pixel_weighted else denom
        ...
        split_rotations = (
            self._rotations[split_mask].expand(2, -1, -1).flatten(end_dim=1)
        )
        offsets = (
        ...
        split_means = (
            self._means[split_mask].expand(2, -1, -1).flatten(end_dim=1) + offsets
        )
        split_scales = split_scales.mul(0.625).log()  # 1 / 1.6 = 0.625
        split_sh_coefficients_0 = (
            self._sh_coefficients_0[split_mask].expand(2, -1, -1, -1).flatten(end_dim=1)
        )
        split_sh_coefficients_rest = (
            self._sh_coefficients_rest[split_mask]
            .expand(2, -1, -1, -1)
            .flatten(end_dim=1)
        )
        split_opacities = (
            self._opacities[split_mask].expand(2, -1, -1).flatten(end_dim=1)
        )

        # incorporate
        n_new_gaussians = n_new_gaussians_duplicate + n_new_gaussians_split
        param_groups = extend_param_groups(
            self.optimizer,
            {
                "means": torch.cat([duplicated_means, split_means]),
                "sh_coefficients_0": torch.cat(
                    [duplicated_sh_coefficients_0, split_sh_coefficients_0]
                ),
                "sh_coefficients_rest": torch.cat(
                    [duplicated_sh_coefficients_rest, split_sh_coefficients_rest]
                ),
    ...
    def mcmc_densification(self, min_opacity: float, cap_max: int) -> None:
        ...
        # relocate
        dead_mask = self._opacities.flatten() <= math.log(
            min_opacity / (1 - min_opacity)
        )
        ...
        if n_dead_gaussians > 0:
            ...
            sampled_indices = torch.multinomial(
                opacities[alive_indices], n_dead_gaussians, replacement=True
            )
            ...
            _, inverse, counts_per_unique = sampled_indices.unique(
                sorted=False, return_inverse=True, return_counts=True
            )
            ...
            adjusted_opacities = adjusted_opacities.clamp(
                min_opacity, 1.0 - torch.finfo(torch.float32).eps
            ).logit()
            ...
            self._sh_coefficients_0[dead_indices] = self._sh_coefficients_0[
                sampled_indices
            ]
            self._sh_coefficients_rest[dead_indices] = self._sh_coefficients_rest[
                sampled_indices
            ]
        ...
        if n_added_gaussians > 0:
            ...
            opacities = self.opacities.flatten()
            sampled_indices = torch.multinomial(
                opacities, n_added_gaussians, replacement=True
            )
    ...
    def as_ply_dict(self) -> dict[str, np.ndarray]:
        ...
        sh_rest = (
            self.sh_coefficients_rest.detach()
            .transpose(1, 2)
            .flatten(start_dim=1)
            .contiguous()
            .cpu()
        ...
        opacities = (
            self.raw_opacities.detach().contiguous().cpu().numpy()
        )  # most viewers expect unactivated opacities
        scales = (
            self.raw_scales.detach().contiguous().cpu().numpy()
        )  # most viewers expect unactivated scales
        rotations = self.rotations.detach().contiguous().cpu().numpy()
        attributes = np.concatenate(
            (means, sh_0, sh_rest, opacities, scales, rotations), axis=1
        )
        ...
        attribute_names = (
            ...
            ]  # remaining SH degree coefficients
...
class FasterGSModel(BaseModel):
    ...
    def get_ply_dict(self) -> dict[str, np.ndarray | list[str]]:
        ...
        splat_render_mode = (
            "mip-0.1" if Framework.config.RENDERER.PROPER_ANTIALIASING else "default"
        )
        data["comments"] = [
            f"SplatRenderMode: {splat_render_mode}",
            "Generated with NeRFICG/FasterGS",
        ]
