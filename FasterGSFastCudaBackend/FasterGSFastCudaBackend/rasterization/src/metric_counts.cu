...
// Builds the same per-tile instance lists as the pruning_scores path (shared preprocess,
// depth sort, instance creation and range extraction), then runs the metric-count blend
// kernel instead of the score kernel. sorting is done separately for depth and tile as
// proposed in https://github.com/m-schuetz/Splatshop
void faster_gs::rasterization::metric_counts(
    ...
    const bool proper_antialiasing)
{
    ...
    CHECK_CUDA(config::debug, "preprocess (metric_counts)")
...
void faster_gs::rasterization::compute_metric_counts(
    std::function<char* (size_t)>& resize_instance_buffers,
