...
namespace faster_gs::rasterization::kernels::metric_counts {
    ...
    // FastGS multi-view consistency count (VCD/VCP importance):
    // for every Gaussian that actually contributes to a high-error pixel (metric_map == 1),
    // accumulate +1 into its per-primitive count. This is a single forward alpha-blend pass
    // (no image output, no gradients). Mirrors the first blend loop of pruning_scores'
    // compute_scores_cu; the preprocessing / instance-building stages are shared with the
    // pruning_scores path (see metric_counts.cu).
