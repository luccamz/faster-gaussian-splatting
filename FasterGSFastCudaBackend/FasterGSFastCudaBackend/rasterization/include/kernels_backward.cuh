...
namespace faster_gs::rasterization::kernels::backward {
        ...
        const bool proper_antialiasing)
    {
        ...
        for (int i = 0; i < config::block_size_blend + 31; ++i) {
            ...
            if (densify_abs) {
                // accumulate abs BEFORE summing over pixels so opposing per-pixel pulls don't cancel (AbsGS)
                dL_dmean2d_abs_accum.x += fabsf(dL_dmean2d.x);
                dL_dmean2d_abs_accum.y += fabsf(dL_dmean2d.y);
            }
